...
function Copy-OptionalPrerequisites([string]$RepoRoot, [string]$StageDir, [string]$WebView2InstallerPath) {
    ...
    if ([string]::IsNullOrWhiteSpace($installer)) {
        $installer = Join-Path $RepoRoot "installer\thirdparty\MicrosoftEdgeWebView2RuntimeInstallerX64.exe"
    }
    ...
    if (Test-Path $installer) {
        ...
        Write-Warning "MicrosoftEdgeWebView2RuntimeInstallerX64.exe was not found. Place it at installer\\thirdparty\\MicrosoftEdgeWebView2RuntimeInstallerX64.exe or pass -WebView2InstallerPath."
    }
}
...
function Compile-InnoInstaller([string]$RepoRoot, [string]$StageDir, [string]$OutputDir, [string]$Version) {
    ...
    $scriptPath = Join-Path $RepoRoot "installer\CEditor.iss"
