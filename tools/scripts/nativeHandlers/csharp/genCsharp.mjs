...
`,
  ...
  //    Trimming is ON (full): brings the self-contained layout from ~80 MB to ~22 MB. The one catch is
  //    that we host this assembly from a NATIVE shim via hostfxr, which the trimmer can't see — so it
  //    defaults the `EnableConsumingManagedCodeFromNativeHosting` feature switch OFF, which trims the
  //    component-activation path AND makes the runtime refuse function-pointer resolution at load time
  //    (FrameworkMissingFailure 0x800080a7). We preempt the ILLink default (it only sets false when the
  //    property is empty) by setting `_EnableConsumingManagedCodeFromNativeHosting=true`, keeping the
  //    switch ON. roots.xml preserves the (tiny) handler assembly so the trimmer never prunes the
  //    [UnmanagedCallersOnly] entry points it can't see are reached from native. Verified at 22 MB.
...
<Project Sdk="Microsoft.NET.Sdk">
  ...
  <PropertyGroup>
    ...
    <!-- Ship a self-contained CoreCLR in pub/ (no user .NET install), trimmed ~80 MB -> ~22 MB. -->
    <SelfContained>true</SelfContained>
    <PublishTrimmed>true</PublishTrimmed>
    <TrimMode>full</TrimMode>
    <!-- We host this assembly from a NATIVE shim (CeHost.c) via hostfxr; the trimmer can't see that and
         would default the native-hosting feature switch OFF (trimming the component-activator path and
         making load-time function-pointer resolution fail with FrameworkMissingFailure 0x800080a7).
         Preempt the ILLink default (it only sets false when empty) so the switch stays ON. -->
