...
| **1. Interactivity model** (§3) | ✅ **Done** — 1A follow-mode `source`/`inflate`/`minTouch` + shared resolver; 1B `makeInteractive` + archetypes; 1C relative dial drag + fine-drag + double-click reset; 1D fine-drag & dbl-click wired into surfaces; 1E Make Interactive tool flyout + layer context action; 1F grab-area halo. 1E/1F re-verified 2026-07-12: the earlier ✅ predated the repo import — the flyout existed but `'interactive'` was missing from DRAW_TOOLS (tool never activated) and `makeInteractive()` was never called; both now wired and browser-verified (see §12.13). | — | Medium — touches factory + materializer + runtime |
...
### 12.11 Status update (2026-07-05, third pass)
- ✅ **scaleInternals + pin labels** shipped after all: implemented at the RESOLVER level (`applyCustomInternalScale` runs last in `resolveInteractiveControl`, after materialize/bindings/states which keep authoring in design space), so rendering and hit-zone resolution stay consistent without touching renderer/player/hit-test code. `Layout.pinned` keeps a part's size/font while its position tracks the scale; toggles in the Transform inspector (Stretch / Scale internals + design-size readout) and the surface object inspector (Pin size). Design size stamped at instantiate.
- ✅ **repeated-buttons + labels generators materialized** (were offered by the editor but silently ignored by the engine): button banks emit per-index setValue zones with value payloads and highlight the active button from the channel; labels emit explicit or min..max-interpolated value labels for linear/vertical/circular scales. `scrollable-content` remains unmaterialized and is labeled so.
- ✅ Fill/gradient seed/open plumbing deduped into `gradientTarget.js` (review item 13); Python execution watchdog (2s, PyErr_SetInterrupt) closes the scripting-guard gap; native-handler pin logging (item 17); bridge lifetime invariant documented (item 19); formatter configs + tmp/ tracking note (items 26/28); §12.5 decomposition started (SurfaceHelpOverlay extracted).
- Still open after this pass: arpeggiator on the array primitive; container/tab-group starters; scrollable-content materialization; terminology rename pass (owner decision); responsive anchors/theme tokens; sharing gallery; debugger native line-stepping; TextEditor/monolith splits continue.

### 12.12 Status update (2026-07-05, §12.11 follow-through)
- ✅ **Array primitive generalized to object items** (`itemFields`: per-field min/max/step/default clamps, pass-through ids, variable length capped by `size`) — the full §12.3 shape.
- ✅ **Arpeggiator re-expressed on the primitive, read side**: the Arp Kit scaffolds an `arpPattern` object-array channel ({note, step, length, velocity}); every sync publishes the normalized block list through it, so the pattern flows through the ordinary channel/published-API machinery. **Write side deliberately deferred**: interactive grid edits and a channel write have no clean precedence (documented at the sync site in `customComponentArpeggiator.js`) — needs its own design pass before both can be pattern sources.
- ✅ **Tab Group container starter** (§12.2 containers): repeated-buttons bank drives a tab enum; rule-driven states swap the visible page. Zero new machinery.
- ✅ **scrollable-content materialized** as a windowed virtual list (count items, rows visible, scroll channel picks the window, optional selection channel with highlighted row). Edge rows snap rather than clip — real clipping would need renderer nesting.
- ✅ **Terminology rename: DECIDED — keep the current names** (owner, 2026-07-05). The glossary overlay is the answer; no label churn. This §12.6 item is closed.
- Still open: arp pattern write-side precedence design; responsive anchors/theme tokens; sharing gallery; debugger native line-stepping; TextEditor/monolith splits.

### 12.13 Status update (2026-07-12, pre-merge audit)
Branch-close review found Phase 1's "Pending user visual QA of 1E/1F" could never pass: the Make Interactive flyout was dead UI. Fixed and browser-verified:
- ✅ **1E draw tool actually wired**: `'interactive'` added to DRAW_TOOLS (before this, `setActiveTool('interactive')` silently no-oped, and `commitDrawnFrame` fell through to a plain inert part — `makeInteractive()` was defined + tested but never called). Drawing now scaffolds the face part (+ range handles) plus the pre-wired channel/behavior/zone set; `I` shortcut; cheatsheet updated.
