...
| **2. Unify the surface** (§4) | ✅ **Done** — 2A merged Public+Public API into one `CustomApiEditor` (Contract/Values modes); 2B de-duped Generators to the surface dock only (`Designer.focusSurfaceDock` jump-link); 2C removed the dead Assets dock tab. Channels confirmed single-home (panel only — not in the surface inspector). Inspector contract verified: surface = Object/Display/Behavior/States/Device. **Deferred (not in §4 bullets, needs sign-off):** the panel "Layers" tab still overlaps the surface layer tree, and panel Channels/Behaviors/Hit Zones are component-wide lists vs the surface's per-object inspector. | 1 (shared inspector) | Low–Medium |
