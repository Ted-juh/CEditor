...
| Document | What it is |
|---|---|
| [Display component futures](display-component-futures.md) | Nine proposals for `LcdDisplay` and `PixelDisplay`, from an audit of what the two actually do. **Six are built** — user-definable glyphs with a 5×8 editor, `editText` verbs, pressable soft keys, `@param:` zones, layouts as a state machine (press in, timeout out, a wrapping cursor), and the pixel scene addressed by element name. A seventh, the CTRL49 framebuffer, was **answered against `CE/src/ControlSurface/` rather than spiked on hardware** and closed. Two remain speculative. The figures in `docs/media/mockup-*.png` are therefore mostly recordings of shipped features rather than pictures of proposals, and the document says which is which. Its most useful parts are the three places the plan was wrong — an index that cannot be a key, an id that cannot be reached, and an interaction model that was better as an exception. Reads alongside the Screen Builder record above; start with that document's non-goals. |
