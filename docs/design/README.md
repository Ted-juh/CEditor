...
| [Product ideas — everything that isn't MIDI](product-ideas.md) | *Nothing built.* The companion to the MIDI record, written in plain English rather than in this repository's usual voice, because it is meant to be readable by anyone. The editor, the exported plugin, the rig, sound design, sharing and onboarding. Leads with the remote panel — a tablet drives the rig without touching MIDI, because the PC keeps the ports and the network carries UI messages, and 275 of the 283 references to the JUCE bridge object sit in one file. Sixteen sections, in two rounds: the first covers the editor, the exported plugin, the rig, sound design, sharing and onboarding; the second adds auto-sampling a synth into a portable instrument, a long scripting section that doubles as a tour of the system for anyone returning to it — seven languages run as themselves, 32 events, one data-declared API kept honest by a parity test — a Lua block editor whose round-trip was spiked against the repo's own .lua files before the proposal was written (`tools/scripts/spikes/lua-blocks/`), and the gap between the script preview and the compiled article, profile coverage as something a user can see, retrospective capture, and the fact that there is no translation system at all. A third round covers the export and packaging side: self-updating panels, which fall out of the template player reading its panel from a file beside it; preflight and a toolchain doctor; what AGPL obliges a panel author to do, which the export path says nothing about today; "where is this component used" over a content hash that already exists; pointing the browser-check harness at the user's components instead of the project's own; one job queue for everything slow; and the synth's reference sheet, printed. A fourth round is mostly gaps found by grepping for their absence: no per-unit concept anywhere, so a profile describes a model while a musician owns a particular Juno-106 that self-oscillates where the next one does not; nowhere to keep the notes every hardware owner writes on paper; three importers and no exporters, when a generated .midnam or Cubase map would put this program's output in front of people who never installed it; no way for a user to say "this does not match my synth"; and a scenery fold that silently and correctly folds 216 of the GAIA's 409 controls with no way for the author to see it. A fifth round covers the Custom Component designer, the largest subsystem in the program and the one the earlier rounds only skimmed — its finding is that the instance/package relationship is already tracked (a content fingerprint, a recorded source, a live comparison) and renders as a read-only card that can say an instance has drifted and do nothing about it, while pull, push, detach, reset and diff are settled prior art; that accessibility there is half-authored already, because every part carries a semantic role for rendering reasons that is almost exactly what a screen reader needs; and that the readiness checklist counts pieces without ever asking whether the component works, next to a 1,541-line test bench that persists nothing. A sixth round covers the sound library and the pattern engine, and is mostly fields that exist and are never spent: branchedFromRecordId records a sound's ancestry and is used only to decide whether a diff button appears; `missing` keeps a vanished source's metadata with no way to relink four hundred of them at once; sonicRefusal records why a sound could not be measured and shows nobody. It also corrects this record's own groove entry in midi-frontier.md — GrooveTemplate, its factory set and applyGrooveTemplate were already built, so only extraction is missing and the idea is much cheaper than first written. Ends with a shortlist, and with a table of ten ideas that turned out to be built already, whose lesson is that the engineering here is usually done and the surfacing is not. |
