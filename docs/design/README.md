...
| [The Assets tab](assets-tab-design.md) | *Built.* Candidate 3: 1,073px, and a defect. `Assets.images` and `Assets.filmstrips` are name-keyed maps picked from two `<select>` dropdowns with one preview at a time, which is no way to choose a picture. The finding worth building for is arithmetic: `InteractivePartRenderer` positions filmstrip frames proportionally rather than in pixels, so a strip whose length does not divide evenly by its frame count renders frames that drift — and nothing in the application checks it, because the check needs the image's natural size and the asset only stores a rounded per-frame one. The tab is one library grid for both kinds, a stage that draws the frames as separate boxes using the renderer's own CSS, and a bar that runs `height % frameCount` and offers the nearest counts that do divide. Two smaller findings came out of the same measurement: the package exporter reads a `width`/`height` off a filmstrip that nothing has ever written, and an image's recorded size goes stale silently. No sliders: step buttons, a typed frame number and the strip itself. Drawn in [`assets-tab-mockups.html`](assets-tab-mockups.html). |
