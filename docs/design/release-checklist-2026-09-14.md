...
| A6. The unit suite still passes with the behaviour fixes in | **MET** | Windows integration re-run at `b81fc112` on 14 Sep: **4,856 tests, 4,856 pass, 0 fail, 0 skipped**, exit 0; frontend production build passed. Seven focused browser suites also passed, with 105 verified rows and zero defects; see `integration-status-2026-09-14.md` |
...
| B3. The 241 unreached properties closed or consciously deferred | **OPEN**, pending the remaining `Icon.tint` decision in §1d | **68** now, and **the editor-and-shared half is closed**: nine suites (`behaviourMouse`, `behaviourTrack`, `behaviourButtons`, `behaviourScreens`, `behaviourLayout`, `behaviourWidgets`, `behaviourLists`, `behaviourShared`, `behaviourAssets`) — the last two re-run 14 Sep after five of §1d's inert options were implemented. The matrix cannot see an `inert`/`closed elsewhere` note, so most of the 68 are rows that HAVE an answer — recorded with their reasons in `residual-issues-2026-09-14.md` §1a–1d — plus the `Designer` block already ruled out of the denominator. Seven §1d options have now been implemented; only `Icon.tint` remains undecided there. The 68 is the independently reproduced Windows/Linux source-scan count, not a count of behavioural failures |
