...
| A6. The unit suite still passes with the behaviour fixes in | **MET** | Windows integration re-run at `43d50bc4` on 14 Sep: **4,871 tests, 4,871 pass, 0 fail, 0 skipped**, exit 0; script-export checks 7 pass / 2 environment skips; frontend production build passed. Eight affected browser ledgers passed with **390 verified rows and zero defects**, plus the 17-row release walkthrough and the focused first-use workflow; see `integration-status-2026-09-14.md` |
...
| Gate | State | Note |
| --- | --- | --- |
| D1. Windows Release build and native tests | **MET** | Fresh Release build from `43d50bc4`; 34/34 native tests passed. Scripting was ON and dev mode OFF |
