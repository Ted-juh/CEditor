...
2. **Unguarded `JSON.parse` on panel open** **[verified]** — `deserializePanel()` at [panelModel.js:184](CE/web/src/CE_Application/stores/panelModel.js:184) calls `JSON.parse(json)` with no try/catch, and the call site in the `onPanelOpened` flow at [panels.js:1101](CE/web/src/CE_Application/stores/panels.js:1101) doesn't guard it either. A corrupted or hand-edited `.cepanel` file throws mid-listener: no user feedback, file appears unopenable. Wrap it, return a structured error, and surface "file is corrupted" in the UI.
