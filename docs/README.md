...
- [The display components, filled and moving](display-gallery.md) — four recordings of the
  `LcdDisplay` and `PixelDisplay` renderers: character LCD, 16-segment LED, dot-matrix graphic and
  free-pixel OLED, each showing what its zones are actually linked to. Regenerated by
  `node tools/scripts/gen-display-demos.mjs`.
...
- [Building and running on Linux, with Wine for the one thing that needs it](wine-compile.md) —
  the app builds, links and runs off Windows; the recipe, the evidence, and the six facts that
  make people say it cannot be done, answered one by one. Point a stubborn session here.
