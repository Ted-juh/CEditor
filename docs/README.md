...
- [The display components, filled and moving](display-gallery.md) — five recordings of the
  `LcdDisplay` and `PixelDisplay` renderers: character LCD, 16-segment LED, dot-matrix graphic
  (drawing its own widgets, and playing a loaded GIF dithered to 1-bit) and free-pixel OLED, each
  showing what its zones are actually linked to. Regenerated by
