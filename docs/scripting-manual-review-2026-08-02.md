...
**Merge note (2026-08-03).** When this branch merged into main it met a larger parallel
scripting line (`claude/scripting-api-*`, 76 commits: the `ce.*` module system, componentVerbs,
parity tests, the API explorer). For the overlapping core files the parallel line's more
advanced implementations were kept — it had independently built its own equivalents of most of
this branch's runtime work (notes, transport, timers, storage, `run`/`emit`, raw MIDI-in,
device events), with its own test coverage. What this branch contributes to the merged main:
the entire docs suite (this log, the generated manual + generator + freshness test, cookbook,
getting-started, index), the doc-error fixes across the spec/plans, the C++ smoke-test
verification work, the phrase pattern-slot/song-mode reducer actions, and the review trail.
This branch's superseded runtime-implementation tests were dropped in the merge; its
Arp/Turing/Looper reducers were dropped in favour of the componentVerbs families. The
generated manual now documents the merged (larger) API surface.
