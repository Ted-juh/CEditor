...
//
// The docked pane stacks editors beside the WebView; this manager is its pop-out counterpart
// and owns any number of DocumentWindows, one per target, each holding that target's editor.
//
// THE SAME ORDERING INVARIANT the pane documents: an editor must die before its processor.
// Each window's half is enforced here — close() and the destructor detach and tear the editor
// down before the window — and the service owns the other half exactly as it does for the pane:
...
// life of the run, so re-floating a part puts its window back where it was. A scrollable,
// independently resizable viewport keeps oversized fixed-size plug-in editors reachable.
