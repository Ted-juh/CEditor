...
def recallPreset(slot, opts=None):
    return __deviceQuery("recallPreset", { "role": __role(opts.get("role") if opts else None), "slot": int(slot) })
def preset(role=None):
    return __deviceQuery("preset", { "role": __role(role) })
