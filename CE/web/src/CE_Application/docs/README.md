...
| Remaining components | Components | ✅ all built | Keyboard · Step Sequencer · Tabbed Container · Scroll Area · Progress Bar · Pitch Wheel · Mod Wheel · Shape, all as their own controlTypes. The three over existing engines (Meter, Ribbon) reuse the engine's section and renderer via `componentFamilies.js`; Shape is a real path renderer reading the designer's existing polygon library. Group and Image already shipped. | [remaining-components.md](./remaining-components.md) |
...
| Value-driven Display | Capability | ✅ built | `Behavior.valueFlow` (twoWay/display/input) + the display maps Bindings could not express: lit cells, peak hold, segments, text-from-value, band thresholds. Read-only turns off drag/wheel/keyboard/focus/host-parameter and pointer events — a display is transparent to clicks, at the cost of hover and tooltips. `utils/displayMode.js` + `utils/displayMaps.js`. | [value-driven-display.md](./value-driven-display.md) |
...
| Ready-made vs custom | Principle | ✅ findings closed | The principle still stands as guidance. Both findings are settled: the XY pad shipped as the Vector Joystick, and the ADSR as the Envelope component. The catalog now carries `overrides` so "ship it as a preset" is a first-class answer rather than advice. | [ready-made-vs-custom.md](./ready-made-vs-custom.md) |
| Component gaps | Survey | ✅ all closed | Every gap this survey named now exists: Knob, Listbox, Number, Text Input, Vector Joystick (XY pad), Envelope (ADSR), Mod Matrix, Meter and Keyboard. Kept as the record of what was missing and why. | [component-gaps.md](./component-gaps.md) |
