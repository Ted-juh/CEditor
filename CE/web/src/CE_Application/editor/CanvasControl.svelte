<script>
  ...
  /** Consume only the click generated by the mouseup that just ended a
   * gesture. A following user click starts with mousedown, which disarms this
   * listener before that click can be swallowed. */
  function swallowNextCanvasClick(afterMouseUp = false) {
    ...
    window.addEventListener('click', (ev) => {
      // Keyboard activation, label forwarding and element.click() have no
      // mouse sequence and must never be consumed by drag cleanup.
    ...
    if (afterMouseUp) setTimeout(cancel, 0);
    else window.addEventListener('mouseup', () => setTimeout(cancel, 0), {
      capture: true,
      once: true,
  ...
  function cancelEditorGesture() {
    const hadGesture = isDragging || isResizing || isRotating;
  ...
  function handleDragEnd(e) {
    ...
    if (moved && !dragSpaceHeld && !carriedNow) {
      const surfacePoint = Number.isFinite(e?.clientX) && Number.isFinite(e?.clientY)
        ? surfacePointFromClient(e.clientX, e.clientY)
        : null;
    ...
    if (e?.type === 'mouseup') swallowNextCanvasClick(true);
  }
  ...
  function handleRotateStart(e) {
    ...
    const p = clientToPanelPoint(surface, e.clientX, e.clientY, scale);
    if (!p) return;
  ...
  function handleRotateMove(e) {
    ...
    const cx = displayX + displayW / 2;
    const cy = displayY + displayH / 2;
    const p = clientToPanelPoint(rotateSurface, e.clientX, e.clientY, scale);
    if (!p) return;
