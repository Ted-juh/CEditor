<script>
  ...
  import {
    TRANSPORT_SOURCES, TRANSPORT_SOURCE_LABELS, MIN_BPM, MAX_BPM,
  ...
  let external = $derived(String(t?.source ?? 'internal') === 'external');
...
{#if t}
  <PropertySection title="Transport">
    <PropertyCell label="Source" span={2} hint="Internal makes this the master clock. MIDI clock in follows an external one — the tempo box then shows what is being received, and the tempo you set here is ignored.">
    ...
    <PropertyCell label="" span={4} hint="Every synced component follows this one clock. Two Transports on a panel are two faces on the same clock, not two clocks.">
      <div class="note">{external ? 'Following MIDI clock in' : 'Master clock'}{t.clockOut === true ? ' · sending clock' : ''}</div>
    </PropertyCell>
  </PropertySection>
  ...
  <PropertySection title="Appearance">
    ...
    <PropertyCell label="Tap tempo" span={1} hint="Tapping the face sets the tempo. Inactive while following an external clock.">
