...
{#if layout}
  ...
  <PropertySection title="Lamp" icon={Lightbulb}>
    ...
    <PropertyCell label="Side" span={1} hint="Which side of the legend the lamp sits on.">
      <select class="val" value={layout.lampSide ?? 'left'} onchange={(event) => set('lampSide', event.target.value)}>
    ...
    <PropertyCell label="Gap" span={1} compact hint="Space between the lamp and the legend.">
      <NumberCell label="Gap" value={layout.lampGap ?? 8} step={1} min={0} max={40} defaultValue={8} onchange={(value) => set('lampGap', value)} />
    </PropertyCell>
  </PropertySection>
{/if}
