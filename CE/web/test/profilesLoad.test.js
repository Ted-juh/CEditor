...
test('the full GAIA is among them, with its requests intact', () => {
  ...
  assert.equal(gaia.requests.length, 27, 'an identity request and one RQ1 per block');
...
test('every GAIA block request is answered by a dump the profile actually defines', () => {
  ...
  assert.equal(dumps.size, 26, 'one dump per addressable block');

  const answered = gaia.requests.filter((r) => r.response?.kind === 'bulkDump');
  assert.equal(answered.length, 26, 'every block request should name its dump');
...
test('the GAIA dumps cover every parameter that has an address', () => {
  // 881 of the 882: master.volume is a CC with no SysEx address, so no dump can carry it. A
